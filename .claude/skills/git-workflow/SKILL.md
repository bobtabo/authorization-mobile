...
以下をすべて満たしたら収束とみなし、監視・対応ループを終了する。

- 直近のCodeRabbitレビュー（3-2で取得した最新レビュー。直前に自分がpushしたコミットとその1つ前のコミットの間の差分に対するレビュー）で、新規のactionableコメントが0件である
- 3-3で洗い出したそれまでの指摘が、すべて「修正済み」か「対応しない理由を明示した上で見送り」のいずれかになっている（放置したまま次のラウンドに進まない）
...
3-1〜3-4の往復が5ラウンド前後で収束しない場合は、無限ループを避けるためユーザーに状況を報告し、判断を仰ぐ（Claude Codeが自律的に往復を続けない）。特に、指摘がコード上の具体的な不具合ではなく本Skillの文言の精度に関する自己言及的な指摘に移ってきた場合は、それ自体が「これ以上往復を続けても収穫逓減である」というシグナルなので、早めにユーザーに報告してよい。
